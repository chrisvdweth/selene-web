...
- **Topic**: a learnable concept with metadata and one notebook mapping.
- **Prerequisite edge**: a directed relationship from prior topic to dependent topic.
- **Learning path**: the prerequisite sequence shown on a topic page.
- **Atlas**: the visual graph and topic library together.
- **Fixture set**: the intentionally small subset of external notebooks rendered in this base.
