...
Run `npm run check`, `npm test`, and `npm run build` for application changes. Data changes must preserve unique topic IDs and valid prerequisite references. Run `.beryl/scripts/check.sh` before handoff when Beryl-managed files change. Do not weaken tests or validate only through screenshots.
